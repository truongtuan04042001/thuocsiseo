...
export default function DanhMucMobile(props) {
    ...
    // });
    useEffect(async () => {
        const stored = sessionStorage.getItem("danhmucnhomhang");
        if (!stored) {
            fetch(API_URL + '/danhmucnhomhang', {
                ...
                .then(data => {
                    //console.log(data)
                    sessionStorage.setItem("danhmucnhomhang", JSON.stringify(data))
                    setNhomhang(data)
                })
        } else {
            var parsestored = JSON.parse(stored)
            setNhomhang(parsestored)
        }

    }, []);
    
    useEffect(async () => {
        const stored = sessionStorage.getItem("danhmuchsx");
        if (!stored) {
            fetch(API_URL + '/danhmuchsx', {
                ...
                .then(data => {
                    //console.log(data)
                    sessionStorage.setItem("danhmuchsx", JSON.stringify(data))
                    setHsx(data)
                })
        } else {
            var parsestored = JSON.parse(stored)
    ...
    return (
        <div className="danh_muc1">
            <hr style={{ margin: 15, marginBottom: 10 }}></hr>

            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 10 }}>
                {/* <img width="22" height="22" src={tune_danhmuc} alt="Bộ lọc tìm kiếm"></img> */}
                <div style={{ marginLeft: 7, fontWeight: 500 , color:'white'}}>Bộ Lọc Tìm Kiếm</div>
            </div>
            <hr style={{ margin: 15, marginBottom: 10 }}></hr>
            <List
                component="nav"
            ...
            >
                <ListItem button onClick={handleClick}>
                    ...
                    {open ? <ExpandLess style={{ color: 'black' }} /> : <ExpandMore style={{ color: 'white', fontSize:16 }} />}
                </ListItem>
                ...
                <ListItem button onClick={handleClick1}>
                    ...
                    {open1 ? <ExpandLess style={{ color: 'black' }} /> : <ExpandMore style={{ color: 'white', fontSize:16  }} />}
                </ListItem>
