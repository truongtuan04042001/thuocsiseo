...
const DetailCombo = (props) => {
    ...
    const LayCombo = async () => {
        if (props.location.state != null || props.location.state != undefined) {
            Combo(props.location.state.item)
        } else {
            await fetch(API_URL + '/comboById/' + props.match.params.IdCombo)
    ...
    return (
        <div>
            <Header2 MaNVvalue={MaNVvalue} capNhatSLGH={totalProductInCart} capNhatTTGH={strTotalPriceOrders} OPModalDN={OPModalDN} callBackApp={DangNhapApp} valueOfModal={openModalDN}   ></Header2>
