...
export default ({ dataFromParent, parentCallback, parentCallback2, parentCallback3, tenNV, maNV }) => {
    ...
    const [columns] = useState([
        ...
        { name: 'TongTien', title: 'Tổng tiền' },
        { name: 'DaTra', title: 'Đã Trả' },
    ...
    const [defaultColumnWidths] = useState([
        ...
        { columnName: 'TongTien', width: 150 },
        { columnName: 'DaTra', width: 150 },
        { columnName: 'CongNo', width: 150 },
    ...
    const [totalSummaryItems] = useState([
        ...
        { columnName: 'TongTien', type: 'max' },
        { columnName: 'TongTien', type: 'sum' },
        { columnName: 'DaTra', type: 'max' },
        { columnName: 'DaTra', type: 'sum' },
        { columnName: 'CongNo', type: 'max' },
        { columnName: 'CongNo', type: 'sum' },

    ]);
    const [currencyColumns] = useState(['TongTien', 'DaTra', 'CongNo']);

    ///////////////
    const getRowId = row => row.MaDHN + ',' + row.DaTra;
    ...
    const [defaultHiddenColumnNames] = useState(['MaDHN']);
    ...
    const commitChanges = async ({ added, changed, deleted }) => {
        ...
        if (changed) {
            if (Object.values(changed)[0] != undefined) {
                const maDHN = Object.getOwnPropertyNames(changed)
                ...
                propertiesObj.forEach(ee1 => {
                    if (ee1 = `TongTien`) {
                        if (parseFloat(obj.TongTien) < 0) {
                            isNan = 1
                            alert(`Đã trả phải lớn hơn 0.`)
                        }
                    }
                });
                if (isNan == 0) {
                    await fetch(`${API_URL}/suaDonHangNhap1Row/${arrsplit[0]}`, {
        ...
        if (deleted) {
            ...
            const arrsplit = deleted[0].split(',')
            await fetch(API_URL + '/xoa1DonHangNhap', {
                method: 'POST',
                headers: {
    ...
    const [editingStateColumnExtensions] = useState([
        ...
        { columnName: 'SDT', editingEnabled: false },
        ...
        { columnName: 'NgayNhapHang', editingEnabled: false },
        { columnName: 'TongTien', editingEnabled: true },
        { columnName: 'DaTra', editingEnabled: false },
        { columnName: 'CongNo', editingEnabled: false },
        { columnName: 'MaDHN', editingEnabled: false },
        ...
        <div>
            <div style={{ width: '100%',  display: 'flex', position: 'relative', backgroundColor:'#B8d8e0' }}>
                ...
                <div style={{ width: '30%', textAlign: 'center', alignSelf: 'center', color: '#197420' }}>
                    <b><h2>Bảng Thống Kê Đơn Hàng Nhập</h2></b>
                </div>
                ...
                <div style={{ width: '30%', textAlign: 'right', alignSelf: 'center', display: 'flex', flexWrap: 'wrap' }}>
                    ...
                    <button className='btn-thongke1' style={{backgroundColor:'#2de183'}} onClick={reLoadDonHang} >
                        <b>Reload</b>
                    </button>
