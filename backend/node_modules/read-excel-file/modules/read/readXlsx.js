...
import { getSharedStrings, getCellValue, getCells, getDimensions, getBaseStyles, getCellStyles, getNumberFormats, getWorkbookProperties, getRelationships, getSheets } from '../xml/xlsx'; // Maps "A1"-like coordinates to `{ row, column }` numeric coordinates.
...
export default function readXlsx(contents, xml) {
  ...
  var values = fileNames.sharedStrings ? parseValues(contents["xl/".concat(fileNames.sharedStrings)], xml) : []; // Default file path for "styles": "xl/styles.xml".

  var styles = fileNames.styles ? parseStyles(contents["xl/".concat(fileNames.styles)], xml) : {};
  ...
  var sheet = parseSheet(contents["xl/".concat(fileNames.sheets[sheetRelationId])], xml, values, styles, properties, options); // If the sheet is empty.
...
function parseFileNames(content, xml) {
  ...
  var addFileNamesInfo = function addFileNamesInfo(relationship) {
  ...
  getRelationships(document).forEach(addFileNamesInfo); // Seems like "sharedStrings.xml" is not required to exist.
